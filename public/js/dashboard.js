...
// const btnincome = document.querySelector('#new-income');
...
async function getContent(){
  const html = await fetch('http://localhost/financiapp/expenses/create').then(res => res.text());
  return html;
}
//
async function getContent(){
...
google.charts.load('current', {'packages':['bar']});
      ...
      async function drawChart() {
        const http = await fetch('http://localhost/financiapp/expenses/getExpensesJSON')
        .then(json => json.json())
        .then(res => res);
